...
Vegas can store its entire internal state (\ie all the information to 
resume an interrupted integration) in an external file.  The state file 
is updated after every iteration.  If, on a subsequent invocation, Vegas 
finds a file of the specified name, it loads the internal state and 
continues from the point it left off.  Needless to say, using an 
...
Once the integration terminates normally, \ie reaches either the 
prescribed accuracy or the maximum number of points, the state file is 
removed unless bit 5 of \Code{flags} explicitly requests that it be
kept.
\end{itemize}
...
M.~Rauch, and A.M.~de~la~Ossa for testing.
